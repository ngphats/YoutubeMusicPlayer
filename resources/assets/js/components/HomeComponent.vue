<template>
  <div class="container">
            ...
            <div class="row align-items-center">
              <div class="col-md-6 mb-3 mb-md-0">
                ...
                <select class="form-select" v-model="player_selected">
                  <option
                    v-for="option in list_player_active"
                    :key="option.socket_id"
                    :value="option.socket_id"
                  >
                    {{
                      option.socket_id === socket.id
                        ? "📱 This Device"
                        : "🖥️ " + option.player_ip
                    }}
                  </option>
        ...
        <div class="card">
          ...
          <div class="card-body p-0">
            ...
            <div
              ...
              :class="{
                'song-item': true,
                'active-song': track.ytid == activeTrack,
              }"
            >
              <div class="d-flex align-items-center p-3">
                ...
                <div class="flex-grow-1 ms-3">
                  <h6
                    class="song-title mb-1"
                    @click="playTrack(track.ytid)"
                    style="cursor: pointer"
                  >
                    {{ track.title }}
                  </h6>
                  <p class="song-message mb-0 text-muted">
                    {{ track.message || "No message" }}
                  </p>
                  <small class="text-muted"
                    >Added: {{ formatDate(track.add_datetime) }}</small
                  >
                </div>
                <div class="flex-shrink-0">
                  <button
            ...
            <form>
              <div class="mb-3">
                ...
                <div
                  ...
                  v-show="errorTmpFormLink.length > 0"
                >
                  {{ errorTmpFormLink }}
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <div class="input-group">
                  ...
                  <textarea
                    class="form-control"
                    id="message-text"
                    v-model="tmpFormMessage"
                    rows="2"
                    placeholder="Add a message for this song..."
                  ></textarea>
                </div>
              </div>
            </form>
    ...
    <div
      ...
      aria-hidden="true"
      v-bind:class="{ show: showSearchModal }"
    >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="search-results" style="min-height: 400px">
              ...
              <div
                v-else-if="
                  searchResultList.length === 0 && search_key.length > 0
                "
              ...
              <div
                v-else-if="searchResultList.length === 0"
                class="text-center p-5 text-muted"
              >
                <i class="fas fa-music fa-3x mb-3"></i>
              ...
              <!-- Search results -->
              <div
                v-for="track in searchResultList"
                :key="track.id"
                class="search-result-item mb-3"
              >
...
export default {
  ...
  methods: {
        ...
        this.playem.getTrackInfo(this.tmpFormLink, (item) => {
          if (undefined === item) {
            this.errorTmpFormLink =
              "Không tìm thấy video, vui lòng kiểm tra lại URL.";
          } else {
            let trackInfo = {
              ytid: item.id,
              thumbnail: item.img,
    ...
    closeSearchModal() {
      this.formClearSubmit();
      this.showSearchModal = false;
      if (this.myModalSearch && typeof this.myModalSearch.hide === "function") {
